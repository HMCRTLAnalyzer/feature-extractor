...
def main():
    ...
    os.system(f"make graph NAME={top} HDL_LANG={lang} MODULE_NAME={top} GRAPH_NAME={top}_ORIGINAL SRC_PATH={orig_src}")

    # If synth_verilog is defined, call makefile on that as well and put results in same directory as above
    if synth_src != "":
        os.system(f"make graph NAME={top} HDL_LANG={lang} MODULE_NAME={top} GRAPH_NAME={top}_SYNTHESIZED SRC_PATH={synth_src}")
